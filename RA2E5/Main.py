...
def step_multiple_modes(sim):
    ...
    if (sim.Rayleigh > 1e7 and abs(wb_zz1_mid) > 1e-5 and (len(sim.iters_new_ts) == 0 or (sim.iteration - sim.iters_new_ts[-1]) > 5)):
        ...
        while (sim.attempt == 0 or repeat_iteration):
            ...
            # Exception handling used for adaptive timestepping. Certainly not ideal but it works.
                ...
                if (sim.ra_growth_coeff > 1.0):
                    ...
                    logger.warning('Negative amplitude in triple wavenumber regime. Removing stable mode from iterative process...')
                    sim.kx_regime -= 1
