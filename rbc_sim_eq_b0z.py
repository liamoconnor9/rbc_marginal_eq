...
b0z_fb = domain.new_field()
b0z_fb.set_scales(evp_scale)

b0z_fb['g'] = b0z
b0z_fb.set_scales(1)
# import matplotlib.pyplot as plt
# plt.pcolormesh(np.copy(b0z_fb['g']))
# plt.savefig(os.path.dirname(os.path.abspath(__file__)) + '/b0z_new')
F = 1
...
problem.parameters['b0z'] = b0z_fb
problem.parameters['Lx'] = Lx
problem.add_equation("dx(u) + wz = 0")
problem.add_equation("dt(b) - P*(dx(dx(b)) + dz(bz))       = -(b0z*w + u*dx(b) + w*bz)")
...
profiles = solver.evaluator.add_file_handler('profiles_eq_b0z', sim_dt=0.1, max_writes=100, mode=fh_mode)
...
checkpoints = solver.evaluator.add_file_handler('checkpoints_eq_b0z', sim_dt=0.5, max_writes=50, mode=fh_mode)
