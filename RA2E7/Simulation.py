    ...
    def get_kx_local_max(self, EV_arg, new_iteration=False, halt_on_work_lm=False):
        ...
        if new_iteration:
            for index in range(1, len(EV_arg) - 1):
                ...
                # else:
                if (abs(EV_arg[index][1]) < 1000*self.growth_tol):
                    kx_local_max.append(EV_arg[index][0])
        else:
            for index in range(1, len(EV_arg) - 1):
                if (EV_arg[index - 1][1] < EV_arg[index][1] and EV_arg[index][1] > EV_arg[index + 1][1] and abs(EV_arg[index][1]) < 1000*self.growth_tol):
                    kx_local_max.append(EV_arg[index][0])
            if (len(kx_local_max) < self.kx_regime):
                ...
                EV_arg.sort(key=lambda x:x[1])
                for i in range(self.kx_regime)[::-1]:
