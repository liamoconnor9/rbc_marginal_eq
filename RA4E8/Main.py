...
def step_multiple_modes(sim):
    ...
    sim.ev_dict = CW.bcast(sim.ev_dict, root=0)
    for i in range(CW.rank, len(sim.kx_marginals), CW.size):
        kx_m = sim.kx_marginals[i]
        logger.warning('marginal? ev: ' + str(sim.ev_dict[kx_m]))
    ...
    if (abs(wb_zz1_mid) > 1e-6 and (len(sim.iters_new_ts) == 0 or (sim.iteration - sim.iters_new_ts[-1]) > 2)):
