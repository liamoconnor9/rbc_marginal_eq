...
def step_multiple_modes(sim):
    ...
    logger.info('Threshold wbzz1 midpoint: ' + str(1e-1))
    if (abs(wb_zz1_mid) > 1e-3 and (len(sim.iters_new_ts) == 0 or (sim.iteration - sim.iters_new_ts[-1]) > 5)):
    ...
    if (min(amp_vec2)[0] < 0):
        ...
        raise ExpectedException('Negative amplitude calculated on second guess: kx' + str(kx_neg))
        ...
        while (sim.attempt == 0 or repeat_iteration):
            ...
            # Exception handling used for adaptive timestepping. Certainly not ideal but it works.
                ...
                if (sim.ra_growth_coeff > 1.0):
                    ...
                    logger.warning('Negative amplitude in multiple wavenumber regime. Removing stable mode and repeating iteration..')
                    sim.kx_regime -= 1
