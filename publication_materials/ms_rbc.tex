...
Then, using the diffused and advected profiles, we compute the eigenvalues $\sigma_{\rm{diff}}$ and $\sigma_{\rm{adv}}$ at $k_x = k_0$.
...
We account for the influence of $N$ advection terms on $N$ eigenvalues (one for each mode) by constructing an eigenvalue matrix $\Sigma_{\rm{adv}} \in \mathbb{R}^{N \times N}$.
For example, the element of $\Sigma_{\rm{adv}}$ at row $i$ and column $j$ is given by the $i$th mode's eigenvalue after a $\Delta t$ period of advection by the $j$th mode's eigenfunctions. 
...
\begin{align*}
    t_{\nu,\delta} &= \frac{L_z}{\mathcal{P}} = 10^4
\end{align*}
\begin{align*}
    \intertext{and another for the entire domain}
    t_{\nu,\mathcal{D}} &= \frac{\delta^2_0}{\mathcal{P}} = 2.8561.
\end{align*}
Empirically, we find $t_{\nu} \approx 200$ suggesting the true kinetic energy decary time-scale is based on a length scale which is longer than the boundary layer height $\delta_0$ but shorter than the domain height $L_z$.
...
A suitable timestep $\Delta t$ must be selected at the start of each iteration.
For sufficiently large $\Delta t$, we find the advective flux terms fail to provide a stabilizing influence.
...
To curtail this, we halt the root-finding algorithms after 20 successive approximations in order to reduce the timestep by a factor of $1.1$.
The iteration is then repeated.
...
Such features are undesired, as they are uncommon in similar calculations \cite{Malkus_1954} and we do not believe they accurately represent the problem's physics.
...
Once these oscillations reach some amplitude, they cannot be eliminated via timestep reduction and the roots of $\sigma_{\rm{max}}(A^2)$ vanish as before.
To avoid this, we measure the extent of the numerical instability preemptively by evaluating the absolute concavity $|\frac{\partial^2}{\partial z^2} \langle w'T' \rangle|_{z=0}|$ of the dominant $k_x = 1.5\pi$ mode.
We find that reducing the timestep $\Delta t$ by the same factor of 1.1 whenever this concavity measure exceeds $10^{-6}$ curtails the problem.
...
\begin{longtable*}{ccccl}
    ...
    \quad\quad\quad $\Ra$	\quad\quad\quad	&	\quad\quad\quad		$N_z$	\quad\quad\quad		&	\quad\quad\quad		$\Nu$	\quad\quad\quad		&	\quad\quad\quad		$\delta_0$	\quad\quad\quad		&	\quad\quad		$k_x$ marginals	\quad\quad\quad\\[0.03cm]
    \hline
