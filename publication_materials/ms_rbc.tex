...
\begin{equation}
    A^2 \approx -\frac{\omega_{\rm{diff}}}{\omega_{\rm{adv}}} \label{EQ:amp_approx}
\end{equation}
...
We use Newton's method to find an amplitude which satisfies our marginal stability tolerance criterion $|\omega_{\rm{max}} (A^2)| < 10^{-9}$.
...
Given a fixed time step, there are now $N$ amplitudes to solve for and $N$ eigenvalues to keep marginally-stable. 
In general, we expect a function $\vec{\omega} \, : \, \mathbb{R}^N \to  \mathbb{R}^N$ to have isolated roots (should they exist). 
To approximate the appropriate amplitude vector $\vec{A^2}$ we assume that for small time steps, diffusion and advection act independently (just as before). We represent the influences of $N$ modes with the linear system
\begin{equation}
    \vec{\omega}_{\rm{diff}} \approx \Omega_{\rm{adv}} \vec{A^2}
    \label{EQ:AN_approx}
\end{equation}
where $\vec{\omega}_{\rm{diff}}$ is the vector of previously marginally-stable eigenvalues obtained by solving the EVP with a diffused mean temperature profile. 
$\Omega_{\rm{adv}} \in \mathbb{R}^{N \times N}$ is the Jacobian matrix of eigenvalues' partial derivatives with respect to the amplitudes (ignoring diffusion and centered at $\vec{A^2} = \vec{0}$).
The $k$th row of $\Omega_{\rm{adv}}$ is then given by the gradient of the $k$th mode's eigenvalue $\nabla \omega_k$.
We perform $N$ periods of advection (one for each mode) followed by EVP solves to approximate $\Omega_{\rm{adv}}$ via first-order finite differences. 
In this way, an arbitrary element of $\Omega_{\rm{adv}}$ represents how a particular mode's eigenvalue responds to the advection of another mode. 
Solving \eq{EQ:AN_approx} gives a mediocre estimate for $\vec{A^2}$. 
We refine our estimate by constructing a new Jacobian matrix via first-order finite differences at the point of our initial estimate (this time including diffusion).
The Jacobian is updated with each guess according to Broyden's method for root-finding in multi-dimensional functions \cite{Broyden}.
We find $\vec{\omega}(\vec{A^2})$ does indeed have a unique root provided the time step is not too large and there are no numerical instabilities, as outlined in appendix \ref{sec:timestep}.
Presumably this is due the dependence of $W(z)$ and $\theta(z)$ on $\bar{T}$.
...
The pairs of modes $k_x = 6\pi, \, 6.5\pi$ and $k_x=23.5\pi, \, 24\pi$ are each associated with a single maximum in our plots of growth rate $\omega$ as a function of $k_x$ (lower right panel of Figure~\ref{fig:flux}).
