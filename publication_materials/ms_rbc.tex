...
\begin{abstract}
    ...
    To study convection in a novel context, we solve Herring's quasilinear form of the Rayleigh-Bénard problem (\citet{herring63}) using a new approach: the perturbations are represented in terms of \textit{marginally-stable eigenmodes}.
    ...
    We find the system tends toward an equilibrium where advection and diffusion sum to a constant.
    The eigenmode amplitudes must be chosen such that the background state maintains marginal stability. 
    We accomplish this by applying Newton's method to the eigenvalues, which themselves are a function of the perturbation amplitudes.
    In this way, marginal stability is maintained and several modes are allowed to simultaneously participate in the evolution of the background state.
    We consider only the countable set of modes whose wavenumbers agree with the problem's periodic domain.
    This approach is unique in that we traverse a marginally-stable manifold to equilibrium, rather than beginning with an unstable state and allowing it to saturate via the advection of a single unstable mode, thereby allowing us to extend Herring's findings to larger $\Ra$. 
    In agreement with \citet{herring63}, we find that only a small subset of the allowable wavenumbers achieve marginal-stability, with a single ``dominant'' mode (whose spatial wavenumber $k_x = 1.5\pi$) participating when $\Ra < 10^6$.
    For $\Ra \geq 10^6$, we find the dominant mode acts in cooperation with higher-wavenumber modes to accommodate the sharp change in diffusive flux which is commonly observed in high-$\Ra$ convection boundary layers.
    We then compare our findings with direct numerical simulations.
    ...
    We find the Nusselt number continues to scale like $\Nu \sim\Ra^{1/3}$ beyond $\Ra = 10^6$.
    ...
    This is due to the thin temperature boundary layers which diffuse heat very effectively, thereby requiring high-velocity advective flows to reach an equilibrium.
\end{abstract}
...
The simplest approach is to initialize the perturbations to low amplitude, and evolve them together with the background (this is, e.g., the approach of \citet{marston2016} and many others).
A different approach was taken in \citet{Beaume_2015}, which computes ECS in parallel shear flows by deriving and solving a quasilinear formulation of the Navier-Stokes equations via multi-scale asymptotic arguments. 
...
In this paper we employ this strategy for the Rayleigh-B\'{e}nard convection problem.
