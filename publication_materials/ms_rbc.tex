...
A two-stage Runge-Kutta timestepper is implemented for the IVP.
...
At some arbitrary time $t = t_0 \geq 0$, we seek to evolve $\bar{T}(z, t_0)$ into a new marginally-stable profile $\bar{T}(z, t_0 + \Delta t)$ according to \eq{EQ:T0_IVP}, using backward Euler to evolve the diffusion term and forward Euler to evolve the advection term. 
...
\begin{equation}
    \partial_t\bar{T} = \langle w'T' \rangle_x
\end{equation}
...
A coarse approximation $\vec{A^2_0}$ for the appropriate amplitude vector $\vec{\tilde{A}^2}$ is given by a generalized form of \eq{EQ:amp_approx}:
\begin{equation}
