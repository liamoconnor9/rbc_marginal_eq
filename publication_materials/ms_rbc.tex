...
\end{equation}
\note{need to define $\langle \cdot \rangle_x$}
...
To solve (\ref{EQ:T0_IVP}) numerically, we need an expression for the perturbation so we can calculate the advective heat flux.
...
Substituting (\ref{EQ:reynolds_dc_u})--(\ref{EQ:reynolds_dc_p}) into (\ref{EQ:motion1})--(\ref{EQ:motion3}) and subtracting (\ref{EQ:T0_IVP}) from the resulting temperature equation, and linearizing gives
\begin{align}
...
In his groundbreaking report \cite{Rayleigh_1916}, Lord Rayleigh observed that (\ref{EQ:linear1})--(\ref{EQ:linear3}) can be manipulated into a separable form with generalized solutions
\begin{align}
...
The linearized system (\ref{EQ:linear1})--(\ref{EQ:linear3}) cannot determine the amplitude of the eigenmodes, $A$.
...
For various $Ra$ and fixed $Pr = 1$, we seek marginally-stable thermal equilibria (MSTE) satisfying $\frac{\partial \bar{T}}{\partial t} = 0$ according to (\ref{EQ:T0_IVP}). 
We employ the \texttt{Dedalus} pseudo-spectral python framework to solve the EVP outlined in Section \ref{sec:model} as well as the IVP (\ref{EQ:T0_IVP}) by representing each field with Chebyshev polynomials \note{cite dedalus}.
...
At some arbitrary time $t = t_0$, we seek to evolve $\bar{T}(z, t_0)$ into a new marginally-stable profile $\bar{T}(z, t_0 + \Delta t)$ according to (\ref{EQ:T0_IVP}), using backward Euler to evolve the diffusion term and forward Euler to evolve the advection term. 
...
Instead, we generalize the advective term in (\ref{EQ:T0_IVP}) to accommodate $N$ simultaneously marginal modes
\begin{equation}
