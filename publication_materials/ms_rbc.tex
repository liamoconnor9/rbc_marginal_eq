...
There exists a substantial body of work pertaining to this specific topic with no general consensus \cite{Malkus_1954}, \cite{Howard_1966}, \cite{Kraichnan} \cite{Spiegel}, \cite{Castaing}, \cite{Grossman}, \cite{Ahlers}. 
...
\begin{align}
    ...
    T(x, z, t) &= \bar{T}(z) + A(t) T'(x, z, t) \label{EQ:reynolds_dc_T}\\
    p(x, z, t) &= \bar{p}(z) + A(t) p'(x, z, t).
\end{align}
where $A(t)$ is some unknown real amplitude and the mean-velocity components vanish due to incompressibility and symmetry. We require that perturbations have no horizontal-average, i.e. an arbitrary perturbation $f'$ obeys
\begin{equation}
...
\end{equation}
thereby constraining $A(t)$. 
Substituting (\ref{EQ:reynolds_dc_T}) into (\ref{EQ:motion3}) yields 
