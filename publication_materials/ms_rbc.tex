...
\end{align*}
Empirically, we find $t_{\nu} \approx 200$ suggesting the true kinetic energy decary time-scale is based on a length scale which is longer than the boundary layer weight $\delta_0$ but shorter than the domain height $L_z$.
...
This is not a numerical instability, rather, it is a limitation of our timestepping algorithm.
...
In general however, the timestep must be limited more strictly to avoid a numerical instability.
...
To avoid this, we measure the extent of the numerical instability preemptively by evaluating the concavity $\frac{\partial^2}{\partial z^2} \langle w'T' \rangle|_{z=0}$ of the dominant $k_x = 1.5\pi$ mode.
We find that reducing the timestep $\Delta t$ by the same factor of 1.1 when this concavity measure exceeds $10^{-6}$ curtails the problem.
