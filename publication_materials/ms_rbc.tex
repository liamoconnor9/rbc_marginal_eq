...
When initializing with different mean temperature profile, we found the the same MSTE, suggesting these equilibria might be unique.

Simulations initialized with the MSTE \eq{EQ:mste_ic} do not undergo an early convective transient period, but have faster flows and greater $\Nu$ when compared with DNS. \note{Do they have larger Nu? It looks like it's the same in the plot!!}
From a dynamical systems perspective, unstable orbits depart from MSTE and approach the global attractor on a viscous time-scale \note{Still reading about this}.
